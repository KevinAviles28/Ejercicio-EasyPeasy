...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write(`Total de películas en cartelera : ${movies.length} peliculas\n\n`);
			let titulos=[];
			movies.forEach(function(title){
				titulos.push(title.title);
			})
            titulos.sort();
			res.write(`Mira nuestras peliculas disponibles:\n`);
			for(i=0;i<titulos.length;i++){
				res.write(`>${titulos[i]}\n`);
			}
			res.write('---------------------------------------------------------------------------\n');
			res.write('Recorda que tambien puedes visitar:\n');
		...
		case '/en-cartelera':
			...
			movies.forEach(function(movies){
				res.write(`\n${movies.title}\n`)
				res.write('\nReseña:\n');
				res.write(`-${movies.overview}\n\n`);
			})
			res.end();
			break;
		case '/mas-votadas':
			res.write('Más votadas.\n\n');
			let nuevaLista=[];//10 pelis
			nuevaLista=movies.filter(function(movies){
				 return movies.vote_average>=7.0;
			})
			let promedio=0;
			nuevaLista.forEach(function(lista){
				promedio+=lista.vote_average;
			})
			promedio= promedio/nuevaLista.length;
			res.write(`Promedio de calificacion de nuestras peliculas:${promedio.toFixed(2)}\n\n`);
			res.write('Peliculas:\n\n')

			nuevaLista.forEach(function(nuevaLista){
				res.write(`Titulo: ${nuevaLista.title}\n`);
				res.write(`Reseña: ${nuevaLista.overview}\n`)
				res.write(`Calificacion: ${nuevaLista.vote_average}\n\n`)
			})
			res.end();
			break;
		case '/sucursales':
			res.write('Nuestras sucursales.\n\n');
			res.write(`Sucursales totales:${theaters.length}\n\n`);
			res.write('Nuestras sucursales:\n\n');
			theaters.forEach(function(sucursales){
				res.write(`Nombre de la sucursal:\n${sucursales.name}\n`);
				res.write(`Direccion:\n${sucursales.address}\n`);
